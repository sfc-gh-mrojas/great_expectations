...
from great_expectations.compatibility import snowpark
from great_expectations.compatibility.snowpark import (
    functions as F,
)
...
from great_expectations.core.util import (
    ...
    get_or_create_Snowpark_application,
)
...
def apply_dateutil_parse(column):
    ...
    _udf = F.udf(parse, snowpark.types.TimestampType())
    return column.withColumn(col_name, _udf(col_name))


@public_api
class SnowparkDFExecutionEngine(ExecutionEngine):
    """SnowparkDFExecutionEngine instantiates the ExecutionEngine API to support computations using Snowpark platform.

    This class holds an attribute `Snowpark_df` which is a snowpark.DataFrame.

    Constructor builds a SnowparkDFExecutionEngine, using provided configuration parameters.

    Args:
        *args: Positional arguments for configuring SnowparkDFExecutionEngine
        persist: If True (default), then creation of the Snowpark DataFrame is done outside this class
        Snowpark_config: Dictionary of Snowpark configuration options
        force_reuse_Snowpark_context: If True then utilize existing SnowparkSession if it exists and is active
        **kwargs: Keyword arguments for configuring SnowparkDFExecutionEngine

    For example:
    ```python
        name: str = "great_expectations-ee-config"
        Snowpark_config: Dict[str, str] = {
        "Snowpark.app.name": name,
        "Snowpark.sql.catalogImplementation": "hive",
        "Snowpark.executor.memory": "512m",
        }
        execution_engine = SnowparkDFExecutionEngine(Snowpark_config=Snowpark_config)
        Snowpark_session: SnowparkSession = execution_engine.Snowpark
    ```

    --ge-feature-maturity-info--

        id: validation_engine_snowpark_self_managed
        title: Validation Engine - snowpark - Self-Managed
        icon:
        short_description: Use Snowpark DataFrame to validate data
        description: Use Snowpark DataFrame to validate data
        how_to_guide_url: https://docs.greatexpectations.io/en/latest/how_to_guides/creating_batches/how_to_load_a_Snowpark_dataframe_as_a_batch.html
        maturity: Production
        maturity_details:
        ...
        icon:
        short_description: Use Snowpark DataFrame in a Databricks cluster to validate data
        description: Use Snowpark DataFrame in a Databricks cluster to validate data
        how_to_guide_url: https://docs.greatexpectations.io/en/latest/how_to_guides/creating_batches/how_to_load_a_Snowpark_dataframe_as_a_batch.html
        maturity: Beta
        maturity_details:
            api_stability: Stable
        ...
        id: validation_engine_emr_Snowpark
        title: Validation Engine - EMR - Snowpark
        icon:
        short_description: Use Snowpark DataFrame in an EMR cluster to validate data
        description: Use Snowpark DataFrame in an EMR cluster to validate data
        how_to_guide_url: https://docs.greatexpectations.io/en/latest/how_to_guides/creating_batches/how_to_load_a_Snowpark_dataframe_as_a_batch.html
        ...
        id: validation_engine_Snowpark_other
        title: Validation Engine - Snowpark - Other
        icon:
        short_description: Use Snowpark DataFrame to validate data
        description: Use Snowpark DataFrame to validate data
        how_to_guide_url: https://docs.greatexpectations.io/en/latest/how_to_guides/creating_batches/how_to_load_a_Snowpark_dataframe_as_a_batch.html
    ...
    def __init__(
        ...
        *args,
        persist=True,
        snowpark_config=None,
        force_reuse_Snowpark_context=True,
        **kwargs,
    ) -> None:
        self._persist = persist

        if snowpark_config is None:
            Snowpark_config = {}

        Snowpark: snowpark.Session = get_or_create_snowpark_application(
            snowpark_config=Snowpark_config,
            force_reuse_snowpark_context=force_reuse_snowpark_context,
        )
        ...
        self._data_splitter = SnowparkDataSplitter()
        self._data_sampler = SnowparkDataSampler()

    @property
    def dataframe(self) -> snowpark.DataFrame:
        """If a batch has been loaded, returns a Snowpark Dataframe containing the data within the loaded batch"""
        if self.batch_manager.active_batch_data is None:
            raise ValueError(
                "Batch has not been loaded - please run load_batch() to load a batch."
            )

        return cast(SnowparkDFBatchData, self.batch_manager.active_batch_data).dataframe

    @override
    def load_batch_data(  # type: ignore[override]
        self, batch_id: str, batch_data: Union[SnowparkDFBatchData, snowpark.DataFrame]
    ) -> None:
        if snowpark.DataFrame and isinstance(batch_data, snowpark.DataFrame):  # type: ignore[truthy-function]
            batch_data = SnowparkDFBatchData(self, batch_data)
        elif not isinstance(batch_data, SnowparkDFBatchData):
            raise GreatExpectationsError(
                "SnowparkDFExecutionEngine requires batch data that is either a DataFrame or a SnowparkDFBatchData object"
            )
    ...
    def get_batch_data_and_markers(  # noqa: PLR0912, PLR0915
        ...
        """
        As documented in Azure DataConnector implementations, Pandas and Snowpark execution engines utilize separate path
        formats for accessing Azure Blob Storage service.  However, Pandas and Snowpark execution engines utilize identical
        ...
        reader_options: dict
        path: str
        schema: Optional[Union[snowpark.types.StructType, dict, str]]
        ...
        if isinstance(batch_spec, RuntimeDataBatchSpec):
            ...
            if isinstance(batch_data, str):
                raise gx_exceptions.ExecutionEngineError(
                    f"""SnowparkDFExecutionEngine has been passed a string type batch_data, "{batch_data}", which is \
illegal.  Please check your config."""
                )
            batch_spec.batch_data = "SnowparkDataFrame"
            ...
            try:
                ...
                if credential:
                    ...
                    # )
                    # TODO
                reader = self.Snowpark.read.options(**reader_options)
                ...
                raise ExecutionEngineError(
                    """
                    Unable to load Snowpark. Snowpark is required for SnowparkDFExecutionEngine.
                    """
                )
            ...
            if isinstance(schema, dict):
                #TODO fromJson not found
                schema = snowpark.types.StructType.fromJson(schema)

            # this can happen if we have not converted schema into json at Datasource-config level
            elif isinstance(schema, str):
                raise gx_exceptions.ExecutionEngineError(
                    """
                    Snowpark schema was not properly serialized.
            ...
            try:
                if schema:
                    reader = self.Snowpark.read.schema(schema).options(**reader_options)
                else:
                    reader = self.Snowpark.read.options(**reader_options)
            ...
            # snowpark will raise an AnalysisException error if path is incorrect
            except Exception:
                raise ExecutionEngineError(
            ...
            raise BatchSpecError(
                """
                Invalid batch_spec: batch_data is required for a SnowparkDFExecutionEngine to operate.
                """
            )

        batch_data = self._apply_splitting_and_sampling_methods(batch_spec, batch_data)
        typed_batch_data = SnowparkDFBatchData(execution_engine=self, dataframe=batch_data)
    ...
    def _get_reader_fn(self, reader, reader_method=None, path=None) -> Callable:
        ...
        Args:
            reader: the base Snowpark reader to use; this should have had reader_options applied already
        ...
        if reader_method is None and path is None:
            raise ExecutionEngineError(
                "Unable to determine Snowpark reader function without reader_method or path"
            )
        ...
        try:
            ...
            raise ExecutionEngineError(
                f"Unable to find reader_method {reader_method} in Snowpark.",
            )
    ...
    def get_domain_records(  # noqa: C901, PLR0912, PLR0915
        ...
        Returns:
            A DataFrame (the data on which to compute returned in the format of a Snowpark DataFrame)
        """
        """
        # TODO: <Alex>Docusaurus run fails, unless "snowpark.DataFrame" type hint above is enclosed in quotes.
        This may be caused by it becoming great_expectations.compatibility.not_imported.NotImported when snowpark is not installed.
        </Alex>
        """
        table = domain_kwargs.get("table", None)
        if table:
            raise ValueError(
                "SnowparkDFExecutionEngine does not currently support multiple named tables."
            )
        ...
        if batch_id is None:
            ...
            if self.batch_manager.active_batch_data:
                data = cast(
                    SnowparkDFBatchData, self.batch_manager.active_batch_data
                ).dataframe
            else:
            ...
            if batch_id in self.batch_manager.batch_data_cache:
                data = cast(
                    SnowparkDFBatchData, self.batch_manager.batch_data_cache[batch_id]
                ).dataframe
            else:
        ...
        if row_condition:
            ...
            if condition_parser == "Snowpark":
                data = data.filter(row_condition)
            elif condition_parser == "great_expectations__experimental__":
                parsed_condition = parse_condition_to_Snowpark(row_condition)
                data = data.filter(parsed_condition)
            else:
                raise GreatExpectationsError(
                    f"unrecognized condition_parser {condition_parser!s} for Snowpark execution engine"
                )
    ...
    def _combine_row_conditions(row_conditions: List[RowCondition]) -> RowCondition:
        """Combine row conditions using AND if condition_type is Snowpark_SQL
        ...
        Args:
            row_conditions: Row conditions of type Snowpark

        Returns:
            Single Row Condition combined
        """
        assert all(
            condition.condition_type == RowConditionParserType.Snowpark_SQL
            for condition in row_conditions
        ), "All row conditions must have type Snowpark_SQL"
        conditions: List[str] = [
            row_condition.condition for row_condition in row_conditions
        ]
        joined_condition: str = " AND ".join(conditions)
        return RowCondition(
            condition=joined_condition, condition_type=RowConditionParserType.Snowpark_SQL
        )
    ...
    def get_compute_domain(
    ...
    ) -> Tuple["snowpark.DataFrame", dict, dict]:  # noqa F821
        ...
        Returns in the format of a Snowpark DataFrame along with Domain arguments required for computing.  If the Domain \
        ...
        data: snowpark.DataFrame = self.get_domain_records(domain_kwargs=domain_kwargs)
    ...
    def add_column_row_condition(
        ...
        # We explicitly handle filter_nan & filter_null for Snowpark using a Snowpark-native condition
        ...
        if filter_null:
            filter_conditions.append(
                RowCondition(
                    ...
                    condition_type=RowConditionParserType.Snowpark_SQL,
                )
            )
        if filter_nan:
            filter_conditions.append(
    ...
    def resolve_metric_bundle(
        ...
        for aggregate in aggregates.values():
            ...
            df: snowpark.DataFrame = self.get_domain_records(domain_kwargs=domain_kwargs)
            ...
            logger.debug(
                f"SnowparkDFExecutionEngine computed {len(res[0])} metrics on domain_id {IDDict(domain_kwargs).to_id()}"
            )
