...
from great_expectations.compatibility.snowpark import functions as F
...
class SnowparkDataSplitter(DataSplitter):
    """Methods for splitting data accessible via SnowparkDFExecutionEngine.

    Note, for convenience, you can also access DatePart via the instance variable
    date_part e.g. SnowparkDataSplitter.date_part.MONTH
    """
    ...
    def split_on_date_parts(
        ...
        for date_part, date_part_value in date_parts_dict.items():
            df = df.filter(
                getattr(F, self._convert_date_part_to_snowpark_equivalent(date_part))(
                    F.col(column_name)
                )
                == date_part_value
            )
    ...
    @staticmethod
    def _convert_date_part_to_snowpark_equivalent(date_part: DatePart | str) -> str:
        """Convert the DatePart to a string representing the corresponding snowflake.snowpark.sql.functions version.

        For example DatePart.DAY -> snowpark.sql.functions.dayofmonth() -> "dayofmonth"

        Args:
            date_part: DatePart representing the part of the datetime to extract or string equivalent.

        Returns:
            String representing the snowpark function to use for the given DatePart.
        """
        date_part = DatePart(date_part)

        snowpark_date_part_decoder: dict = {
            DatePart.YEAR: "year",
        ...
        return snowpark_date_part_decoder[date_part]
    ...
    def split_on_whole_table(
        ...
        Args:
            df: Snowpark DataFrame that will be returned
    ...
    def split_on_column_value(
        ...
        Args:
            df: Snowpark DataFrame to be filtered.
    ...
    def split_on_divided_integer(
        ...
        res = (
            df.withColumn(
                "div_temp",
                (F.col(column_name) / divisor).cast(snowpark.types.IntegerType()),
            )
    ...
    def split_on_mod_integer(df, column_name: str, mod: int, batch_identifiers: dict):
        ...
        res = (
            df.withColumn(
                "mod_temp", (F.col(column_name) % mod).cast(snowpark.types.IntegerType())
            )
    ...
    def split_on_multi_column_values(df, column_names: list, batch_identifiers: dict):
        ...
        for column_name in column_names:
            ...
            if not value:
                raise ValueError(
                    f"In order for SnowparkDFExecutionEngine to `_split_on_multi_column_values`, "
    ...
    def split_on_hashed_column(
        ...
        try:
            ...
            raise (
                gx_exceptions.ExecutionEngineError(
                    f"""The splitting method used with SnowparkDFExecutionEngine has a reference to an invalid hash_function_name.
        ...
        encrypt_udf = F.udf(_encrypt_value, snowpark.types.StringType())
        res = (
